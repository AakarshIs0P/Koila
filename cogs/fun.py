...
class Fun_Commands(commands.Cog):
    ...
    @app_commands.command(name="duck", description="Posts a random duck 🦆")
    ...
    @app_commands.command(name="coffee", description="Posts a random coffee ☕")
    ...
    @app_commands.command(name="beer", description="Offer someone a beer! 🍺")
    ...
    async def slash_beer(self, interaction: discord.Interaction, user: discord.Member = None, reason: str = ""):
        ...
        if user.bot:
            return await interaction.response.send_message(f"**{user.name}** is a bot — bots don't drink 🤖")
        msg_text = f"**{user.mention}**, you have a 🍺 offer from **{interaction.user.name}**!"
        if reason: msg_text += f"\n> {reason}"
    ...
    @app_commands.command(name="hotcalc", description="How hot is someone? 🔥")
